...
function KA.AbilityTracker(
	...
	if IsUnitInCombat("player") then
		local _, _, isTank = GetGroupMemberSelectedRole("player")
		-- Trash
		if abilityId == KA.AbilityIDs.Infuse then
			cfmutils.Debug(
				"Vampire Infuser casts Infuse. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
			...
			if cfmutils.savedVars.KynesTrackInfuse and result == ACTION_RESULT_BEGIN then
				...
				local id = CombatAlerts.CastAlertsStart(
					abilityId,
					"Vampire Infuser",
					hitValue,
					CombatAlerts.vars.maxCastMS,
					{ 1, 0.7, 0, 0.5 },
					{
						hitValue,
						"Interrupt",
		...
		elseif abilityId == KA.AbilityIDs.HealingFumes then
			cfmutils.Debug(
				"Apothecary casts Healing Fumes. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
		...
		elseif abilityId == KA.AbilityIDs.Frenzy then
			...
			if (currentTargetHP / maxTargetHP) > 0.4 then
				cfmutils.Debug(
					"Lt.Njordal casts Blood Frenzy. Result = "
						.. result
						.. ", HitValue = "
						.. hitValue
						.. ", SourceName = "
		...
		elseif abilityId == KA.AbilityIDs.SanguineGrasp then
			cfmutils.Debug(
				"Lt.Njordal casts Sanguine Grasp. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
		...
		elseif
			abilityId == KA.AbilityIDs.BloodCleave
			and targetType ~= COMBAT_UNIT_TYPE_PLAYER
			and cfmutils.savedVars.KynesBloodCleave
		then
		...
		elseif abilityId == KA.AbilityIDs.FirstFloorBreak then
			cfmutils.Debug(
				"Lord Falgravn casts Channel on the Conduits. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
			...
			if cfmutils.savedVars.KynesPositionOveride then
				KA.Pos = OSI.CreatePositionIcon(
					KA.Positions[cfmutils.savedVars.KynesPosition][1],
					...
					function(data)
						...
						data.offset = cfmutils.savedVars.MarkerOffset
							+ cfmutils.savedVars.MarkerBob * math.sin(GetGameTimeMilliseconds() / 1000 * 2)
					end
				)
			else
				KA.Pos = OSI.CreatePositionIcon(
					KA.Positions[KA.PlayerPos[GetDisplayName()]][1],
					KA.Positions[KA.PlayerPos[GetDisplayName()]][2],
					KA.Positions[KA.PlayerPos[GetDisplayName()]][3],
					cfmutils.savedVars.MarkerStyle,
					OSI.GetIconSize() * cfmutils.savedVars.MarkerSize,
					cfmutils.savedVars.MarkerColor,
					cfmutils.savedVars.MarkerOffset,
		...
		elseif abilityId == KA.AbilityIDs.Hemorrhage then
			cfmutils.Debug(
				"Falgravn casts Hemorrhage. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
		...
		elseif abilityId == KA.AbilityIDs.SecondFloorBreak then
			cfmutils.Debug(
				"Lord Falgravn casts Floor Break MID. Result = "
					.. result
					.. ", HitValue = "
					.. hitValue
					.. ", SourceName = "
