...
function cfmutils.zoneCheck()
	...
	if cfmutils.savedVars.enabled then
		...
		elseif zoneId == cfmutils.zones["Asylum Sanctorium"] then
			cfmutils.ASActivate()
		elseif zoneId == cfmutils.zones["Dreadsail Reef"] then
			cfmutils.DSRActivate()
		else
			cfmutils.Deactivate()
		end
	end
...
function cfmutils.CheckActivation()
	if not cfmutils.savedVars.enabled then
		...
		cfmutils.EM:RegisterForEvent(
			cfmutils.name .. "playerActive",
			EVENT_PLAYER_ACTIVATED,
			cfmutils.OnPlayerActivated
		)
		cfmutils.zoneCheck()
	end
end
...
function cfmutils.UpdateArrowStyle()
	LibSimpleArrow.ApplyStyle(
		cfmutils.savedVars.ArrowStyle,
		cfmutils.savedVars.ArrowColor,
		cfmutils.savedVars.ArrowSize
	)
end
