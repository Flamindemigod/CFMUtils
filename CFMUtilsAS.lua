...
AS.AbilityIDs = {
	...
	BossEvent = 10298,
	Enrage = 101354,
	LlothisSoulStainedCorruption = 95585, -- Llothis Interrupt
}
AS.SpawnTimers = {}
AS.standingInAoe = {
...
function AS.AbilityTracker(
	...
	if abilityId == AS.AbilityIDs.StormOfTheHeavens and cfmutils.savedVars.AsylumShowKite then
		if OSI and OSI.CreatePositionIcon then
			...
			if not AS.LaneStart then
				AS.LaneStart = OSI.CreatePositionIcon(
					AS.Lanes[cfmutils.savedVars.AsylumLane].Start[1],
					AS.Lanes[cfmutils.savedVars.AsylumLane].Start[2],
					AS.Lanes[cfmutils.savedVars.AsylumLane].Start[3],
	...
	elseif
		cfmutils.damageEvents[ActionResult]
		and targetType == COMBAT_UNIT_TYPE_PLAYER
		and AS.standingInAoe[abilityId]
	then
		...
		if AS.ProtectorLocations[targetUnitId] then
			cfmutils.Debug(
				"AbilityID = "
					.. abilityId
					.. ", Protector Location = "
					.. AS.ProtectorLocations[targetUnitId]
					.. ", TargetUnitID = "
					.. targetUnitId
			)
		else
			cfmutils.Debug(
				"AbilityID = " .. abilityId .. ", Protector Location = Unknown, TargetUnitID = " .. targetUnitId
			)
		end
	end
end

function AS.EffectUpdate(
	_,
	changeType,
	effectSlot,
	effectName,
...
function AS.BossesChanged()
	local boss = cfmutils.GetBossName()
...
function cfmutils.ASActivate()
	if not AS.init then
		...
		cfmutils.EM:AddFilterForEvent(
			cfmutils.name .. "PowerUpdate",
			EVENT_POWER_UPDATE,
